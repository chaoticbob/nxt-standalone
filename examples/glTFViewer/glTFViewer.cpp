...
namespace {
    void initBuffers() {
        defaultBuffer = device.CreateBufferBuilder()
            .SetUsage(nxt::BufferUsageBit::Vertex | nxt::BufferUsageBit::Index | nxt::BufferUsageBit::Mapped)
            .SetSize(256)
            .GetResult();
        ...
        for (const auto& bv : scene.bufferViews) {
            ...
            nxt::BufferUsageBit usage = nxt::BufferUsageBit::Mapped;
    ...
    void initTextures() {
...

        {
            ...
            commands.CopyBufferToTexture(staging, oTexture, 0, 0, 0, 1, 1, 1, 0);
        ...
        for (const auto& t : scene.textures) {
            ...
            commands.CopyBufferToTexture(staging, oTexture,
                    0, 0, 0, iImage.width, iImage.height, 1, 0);

            textures[iTextureID] = oTexture.CreateTextureViewBuilder().GetResult();
        }
        auto cmdbuf = commands.GetResult();

...
namespace backend {
    ...
    void BindGroupBuilder::SetBufferViews(uint32_t start, uint32_t count, BufferViewBase* const * bufferViews) {
        ...
        for (size_t i = start, j = 0; i < start + count; ++i, ++j) {
            ...
            if (!(bufferViews[j]->GetBuffer()->GetUsage() & requiredBit)) {
                device->HandleError("Buffer needs the correct usage bit");
                return;
            }
        }
    ...
    void BindGroupBuilder::SetTextureViews(uint32_t start, uint32_t count, TextureViewBase* const * textureViews) {
        ...
        for (size_t i = start, j = 0; i < start + count; ++i, ++j) {
            ...
            if (!(textureViews[j]->GetTexture()->GetUsage() & nxt::TextureUsageBit::Sampled)) {
                device->HandleError("Texture needs the sampled usage bit");
                return;
            }
        }
